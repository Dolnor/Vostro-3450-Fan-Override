...
DefinitionBlock ("SSDT-4.aml", "SSDT", 2, "DELL ", "PollDevc", 0x00001000)
{
    ...
    Scope (\_SB.PCI0.LPCB)
    {
        Device (SMCD)
        {
			...
			Method (FAN0, 0, NotSerialized)    // Read the status of fan mode and report as fake fan
			{
				Acquire (^^EC0.MUT0, 0xFFFF)
				...
				Release (^^EC0.MUT0)
                 ...
                 {
                    Return (0x01)             // If 1 is returned - fan mode auto
                 }
                 If (LEqual (Local0, Zero))
                 {
                    Return (0x02)             // If 0 is returned - fan mode locked
                 }
                 Return (0x00)             
			}
        ...
        Device (PLLD)
        {
            ...
            Method (FCTL, 0, NotSerialized)         // Fan Control Method
            {
                ...
                Store (^^SMCD.FAN1(), Local2)       // Store fan speed (both high and low order) as Local2
                Acquire (^^EC0.MUT0, 0xFFFF)        // Set EC Lock
                Store (^^EC0.FLVL, Local1)          // Store Fan Level as Local1
                Release (^^EC0.MUT0)                // Release EC Lock

                If (LNotEqual(Local3, Zero))   // If if not running on battery
				{
					...
					Else                                 // If Fan Mode is Manual
					{
						...
						If (LEqual (Local2, 0x00)) // If fan is off for some reason but mode is manual
						{
							Acquire (^^EC0.MUT0, 0xFFFF)
							Store (One, ^^EC0.TCTL)
							Release (^^EC0.MUT0)
						...
						If (LGreater (Local2, 0xBB8)) // If fan rpm is higher than requested
						{
							Acquire (^^EC0.MUT0, 0xFFFF)
							Store (One, ^^EC0.TCTL)
							Release (^^EC0.MUT0)
				...
				Else // Restore Automatic control if suddenly running on battery
				{
					Acquire (^^EC0.MUT0, 0xFFFF)
					Store (One, ^^EC0.TCTL)
					Release (^^EC0.MUT0)
                ...
                Return (Local1) // Return Fan Level to determine if fan is locked
            }
        }
    }
}